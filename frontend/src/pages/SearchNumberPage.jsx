...
import {
  ...
  Modal, Row, Col, Checkbox, Empty, Tooltip
} from 'antd';
import { 
...
import StyledButton from '../components/StyledButton'; // <-- Sử dụng StyledButton
...
const GatewayCleanupPage = () => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        setSelectedKeys(new Set(resultsWithKey.map(r => r.key))); // Mặc định chọn hết
      } else {
      ...
      const successModal = modalApi.success({
        ...
        content: (
          <div>
            ...
            <div style={{ 
                marginTop: '10px', padding: '10px', background: '#f5f5f5', 
                borderRadius: '6px', maxHeight: '150px', overflowY: 'auto', border: '1px solid #d9d9d9'
            }}>
  ...
  // --- RENDER CARD (HÌNH TRÒN - BO GÓC MẠNH) ---
  const renderCard = (item) => {
    const isSelected = selectedKeys.has(item.key);
    const details = [];
    
    // Tóm tắt thông tin tìm thấy
      ...
      <Col xs={24} sm={12} md={8} lg={6} key={item.key}>
        ...
        >
            ...
            <Card
                ...
                style={{ 
                    height: '100%', 
                    borderRadius: '24px', // <-- Bo góc cực mạnh (giống hình tròn/bầu dục)
            ...
            >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                        <Tag color="blue" style={{ marginRight: 0, borderRadius: '12px' }}>{item.server_name}</Tag>
                        <Tag color={item.type === 'MG' ? 'pink' : 'orange'} style={{ marginRight: 0, borderRadius: '12px' }}>{item.type}</Tag>
                    </div>
                    ...
                    <Tooltip title="View Details">
                        <StyledButton 
                            ...
                            style={{ minWidth: '32px', height: '32px', padding: 0, marginTop: '-4px', color: '#8c8c8c' }}
                            shape="circle" // Nút View tròn
                        />
                    </Tooltip>
                </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Card style={{ marginBottom: 20, borderRadius: '24px' }}> {/* Card nhập liệu cũng bo tròn mạnh */}
        ...
        <StyledButton 
          ...
          loading={loading} 
          size="large"
          shape="round" // Nút Scan hình tròn
        >
          Scan for Cleanup
        </StyledButton>
      </Card>
        ...
        {scanResults.length > 0 ? (
          <>
            <div style={{ 
                ...
                borderRadius: '24px', // Thanh công cụ cũng bo tròn
                ...
                <StyledButton 
                    ...
                    size="large"
                    shape="round" // Nút Clean hình tròn
                >
      ...
      <Modal
        ...
        footer={[
            <StyledButton key="close" onClick={() => setViewDetailRecord(null)} shape="round">Close</StyledButton>
        ]}
      >
        {renderDetailContent()}
      </Modal>
